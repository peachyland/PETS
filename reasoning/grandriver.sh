...
MY_CMD="python aime24.py --model_name ${MODEL_NAME} --data_path ${DATA_PATH} --out ${OUT_PATH} --n ${N} --max_new_tokens ${MAX_NEW_TOKENS} --group_size ${GROUP_SIZE} --group_id ${GROUP_ID} --temperature ${TEMPERATURE} --top_p ${TOP_P} --seed ${SEED} --batch_size ${BATCH_SIZE}"
