...
def create_model_tokenizer(model_name: str):
    ...
    try:
        ...
        sys.exit(f"Error loading model/tokenizer for '{model_name}' - {e}")
...
def process_question(
    ...
    *,
    ...
    # _top_p = 1 if _needs_fixed_sampling(model_name) else top_p
    # _extra_body = _build_extra_body(model_name)
...
def run_inference(
    ...
    with open(output_path, "w", encoding="utf-8") as f, \
        ...
        for item in dataset:
        # for res in dataset.map(process_fn):
            res = process_fn(item)
